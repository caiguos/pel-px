Polymer({
  ...
  properties: {
     ...
     * the <pel-px-chart-series-*> web component to line, bar, histogram, etc.
     *
          ...
          if (evt.originalEvent && evt.originalEvent.shiftKey) {
            ...
            this.xAxis[0].addPlotBand({
              ...
              label: {
                ...
                text: "<i class='fa fa-lg fa-search-plus u-mr- style-scope pel-px-chart'" +
                "onclick='" +
                "var wc=this;" +
                "while(!wc.chart) {" +
                "wc = wc.parentNode" +
                ...
                "</i>" +
                "<i class='fa fa-lg u-mr- fa-times style-scope pel-px-chart'" +
                "onclick='" +
                "var wc=this;" +
                "while(!wc.chart) {" +
                "wc = wc.parentNode" +
  ...
  _initializeMargins: function() {
    ...
    if (margin && margin[0] === 50) {//adjust top margin if zoom controls not present
      var controls = Polymer.dom(this).querySelector('pel-px-chart-controls');
  ...
  _addAxisAndSeries: function() {
    ...
    var axisEls = Polymer.dom(this).querySelectorAll("pel-px-chart-yaxis");
    var axisElsProcessed = 0;
    if (!axisEls || axisEls.length === 0) {
      //update the default yAxis with our own default options...
      this.defaultYAxisConfig = this.defaultYAxisConfig || document.createElement("pel-px-chart-yaxis");
  ...
  addInitialSeries: function() {
    ...
    var _this = this;
    var seriesEls = [];
    ["pel-px-chart-series-line", "pel-px-chart-series-histogram", "pel-px-chart-series-bar",
     "pel-px-chart-series-scatter", "pel-px-chart-series-timeline"].forEach(function(series) {
  ...
  addSeries: function(seriesConfig, noRedraw) {
    if (seriesConfig && this.hasSeries(seriesConfig.id)) {
      ...
      if (!seriesConfig) {
        this.defaultSeriesConfig = this.defaultSeriesConfig || document.createElement("pel-px-chart-series");
      ...
      events.forEach(function(event) {
        var eventConfig = {
          ...
          label: {
            ...
            text: "<span class='style-scope pel-px-chart data-event-icon' title='" + event.label + "' style='cursor:pointer; display:block; margin-top: -1.10rem; margin-left:-0.87rem'><i class='fa fa-lg fa-exclamation-triangle style-scope pel-px-chart'></i> </span>"
          }
        };
      ...
      switch (tooltipType) {
        case 'condensed':
          return {
            ...
            padding: 0,
            formatter: function() {
              var s = '<div class="flex flex--right pel-px-chart-tooltip style-scope pel-px-chart">';
              if(this.points) {
